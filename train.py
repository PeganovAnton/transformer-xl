...
def evaluate_and_log(eval_iter, optimizer, split, train_step=-1):
...
def train(va_iter, optimizer, scheduler):
    ...
    for batch, (data, target, seq_len) in enumerate(tr_iter):
        ...
        if train_step % args.eval_interval == 0:
            evaluate_and_log(va_iter, optimizer, 'val', train_step)
...
def main():
    ...
    # Eval one more time.
    evaluate_and_log(va_iter, optimizer, 'val', train_step=-1)
    ...
    # Run on test data.
    evaluate_and_log(te_iter, optimizer, 'test', -1)
